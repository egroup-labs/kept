...
import { cmdKgGetGraph, agentChat, cmdKgIndexVault, getConfig, getConversation, listConversations, isTauri } from "../lib/tauri-api";
import type { AgentMessage, GraphData, GraphEdge, GraphNode, ToolExecution } from "../lib/types";
...
export default function GraphExplorer({
  ...
  const [kgChatMessages, setKgChatMessages] = useState<KgChatMessage[]>([]);
  const [kgChatInput, setKgChatInput] = useState("");
  const [kgChatLoading, setKgChatLoading] = useState(false);
  const [kgChatProgress, setKgChatProgress] = useState<string | null>(null);
  const kgChatScrollRef = useRef<HTMLDivElement>(null);
  const kgChatInputRef = useRef<HTMLInputElement>(null);
  const agentHighlightIndicesRef = useRef<Set<number>>(new Set());
  const agentHighlightUntilRef = useRef(0);

  // Auto-scroll chat messages
  useEffect(() => {
    if (kgChatScrollRef.current) {
      kgChatScrollRef.current.scrollTop = kgChatScrollRef.current.scrollHeight;
    }
  }, [kgChatMessages, kgChatProgress]);

  // Listen for kg-agent-progress events
  useEffect(() => {
    if (!isTauri) return;
    let unlisten: (() => void) | null = null;
    (async () => {
      const { listen } = await import("@tauri-apps/api/event");
      unlisten = await listen<{ stage: string; tool_name?: string; iteration: number }>("kg-agent-progress", (event) => {
        const { stage, tool_name } = event.payload;
        if (stage === "thinking") {
          setKgChatProgress("Thinking...");
        } else if (stage === "tool_call" && tool_name) {
          setKgChatProgress(`Running ${tool_name}...`);
        } else {
          setKgChatProgress(stage);
        }
      });
    })();
    return () => { unlisten?.(); };
  ...
  const sendKgChat = useCallback(async () => {
    const text = kgChatInput.trim();
    if (!text || kgChatLoading) return;

    const userMsg: KgChatMessage = { role: "user", content: text };
    setKgChatMessages(prev => [...prev, userMsg]);
    setKgChatInput("");
    setKgChatLoading(true);
    setKgChatProgress("Thinking...");

    try {
      // Build message history for the agent
      const history: AgentMessage[] = [...kgChatMessages, userMsg].map(m => ({
        role: m.role,
        content: m.content,
      }));

      // Get configured model for agentic tasks
      const config = await getConfig();
      const agenticModels = config.model_assignments?.["agentic"] ?? [];
      const firstModel = agenticModels[0];
      if (!firstModel) {
        setKgChatMessages(prev => [...prev, {
          role: "assistant",
          content: "No model configured for agentic tasks. Please set one in Settings → Model Assignments.",
        }]);
        return;
      }

      const response = await agentChat({
        platform: firstModel.provider,
        model: firstModel.model,
        messages: history,
        event_channel: "kg-agent-progress",
      });

      setKgChatMessages(prev => [...prev, {
        role: "assistant",
        content: response.content,
        toolExecutions: response.tool_executions.length > 0 ? response.tool_executions : undefined,
      }]);

      // If the agent successfully modified the graph, refresh it
      const writeTools = new Set(["add_edge", "remove_edge", "add_entity", "remove_node"]);
      const hasSuccessfulWrite = response.tool_executions.some(
        t => writeTools.has(t.tool_name) && !t.result.startsWith("Error")
      );
      if (hasSuccessfulWrite) {
        // Dispose old scene before rebuilding to avoid WebGL context corruption
        cleanupRef.current?.();
      ...
      setKgChatLoading(false);
      setKgChatProgress(null);
    }
  }, [kgChatInput, kgChatLoading, kgChatMessages]);
    ...
    <div className="relative flex w-full h-full overflow-hidden" style={{ minHeight: 0, userSelect: "none", WebkitUserDrag: "none" } as React.CSSProperties} onDragStart={e => e.preventDefault()}>
      ...
      <div
        ...
        style={{
          ...
          bottom: kgChatOpen ? 72 : 24,
          width: kgChatOpen ? "min(480px, calc(100% - 48px))" : "auto",
          zIndex: 25,
          ...
          >
            ...
            <div
              style={{
                ...
                padding: "10px 14px",
                borderBottom: "1px solid rgba(195, 236, 255, 0.06)",
              }}
            >
              <span style={{
                fontSize: 9,
              ...
              <button
                ...
                onMouseEnter={e => { e.currentTarget.style.color = "rgba(195, 236, 255, 0.8)"; e.currentTarget.style.borderColor = "rgba(195, 236, 255, 0.2)"; }}
                onMouseLeave={e => { e.currentTarget.style.color = "rgba(195, 236, 255, 0.5)"; e.currentTarget.style.borderColor = "rgba(195, 236, 255, 0.1)"; }}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                  <path d="M2 20h20" stroke="currentColor" strokeWidth="2.5" />
                </svg>
              </button>
            </div>
            ...
            >
              {kgChatMessages.length === 0 && !kgChatLoading && (
                <div style={{
                  fontSize: 12,
                  ...
                  Ask the agent to manage your knowledge graph...
                </div>
              )}
              {kgChatMessages.map((msg, i) => (
                <div key={i} style={{
                  display: "flex",
                  flexDirection: "column",
                  ...
                  <div style={{
                    fontSize: 12,
                    lineHeight: 1.5,
                    color: msg.role === "user" ? "rgba(195, 236, 255, 0.85)" : "rgba(195, 236, 255, 0.75)",
                    background: msg.role === "user" ? "rgba(195, 236, 255, 0.06)" : "transparent",
            ...
            <div style={{
              padding: "8px 10px",
              borderTop: "1px solid rgba(195, 236, 255, 0.06)",
              display: "flex",
              gap: 8,
            }}>
              <input
                ...
                type="text"
                value={kgChatInput}
                onChange={e => setKgChatInput(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendKgChat(); } }}
                placeholder="Manage the knowledge graph..."
                disabled={kgChatLoading}
                style={{
                  flex: 1,
              ...
              <button
                onClick={sendKgChat}
                disabled={kgChatLoading || !kgChatInput.trim()}
                className="cursor-pointer"
                style={{
                  background: kgChatInput.trim() ? "rgba(217, 119, 87, 0.15)" : "rgba(195, 236, 255, 0.04)",
                  border: `1px solid ${kgChatInput.trim() ? "rgba(217, 119, 87, 0.25)" : "rgba(195, 236, 255, 0.08)"}`,
                  borderRadius: 6,
                  padding: "0 12px",
                  color: kgChatInput.trim() ? "rgba(217, 119, 87, 0.7)" : "rgba(195, 236, 255, 0.25)",
                  fontSize: 11,
                  fontWeight: 500,
      ...
      {/* Bottom center — KG Agent toggle */}
      {!kgChatOpen && sceneReady && !graphEmpty && (
        <div
          className="absolute bottom-0 left-1/2 pointer-events-none"
          style={{ transform: "translateX(-50%)", padding: "0 16px clamp(20px, 4vh, 32px)", zIndex: 15 }}
        >
          <button
            onClick={() => { setKgChatOpen(true); setTimeout(() => kgChatInputRef.current?.focus(), 100); }}
            ...
            style={{
              ...
              boxShadow: "0 0 24px rgba(195, 236, 255, 0.04)",
            }}
            onMouseEnter={e => {
              e.currentTarget.style.borderColor = "rgba(195, 236, 255, 0.3)";
              e.currentTarget.style.color = "rgba(195, 236, 255, 0.8)";
              e.currentTarget.style.background = "rgba(2, 10, 13, 0.75)";
              e.currentTarget.style.boxShadow = "0 0 32px rgba(195, 236, 255, 0.08)";
            }}
            onMouseLeave={e => {
              e.currentTarget.style.borderColor = "rgba(195, 236, 255, 0.15)";
              e.currentTarget.style.color = "rgba(195, 236, 255, 0.5)";
              e.currentTarget.style.background = "rgba(2, 10, 13, 0.65)";
              e.currentTarget.style.boxShadow = "0 0 24px rgba(195, 236, 255, 0.04)";
            }}
          >
            <svg width="14" height="14" viewBox="0 0 16 16" fill="none" style={{ opacity: 0.6 }}>
              <path d="M2 2h12v9H5l-3 3V2z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />
            </svg>
            KG Agent
          </button>
        </div>
